...
\begin{figure}[!h]
 ...
 \captionof{figure}{Side A Bin migration due to the segmentation size of VIDARR. In this example side B has no $\phi$ component and so any value on side A determines $\phi$ exclusively warping the distribution.} 
...
\begin{figure}[!h]
 ...
 \captionof{figure}{Side B Bin migration due to the segmentation size of VIDARR. In this example side A has no $\phi$ component and so any value on side B determines $\phi$ exclusively warping the distribution.} 
...
\begin{minipage}{.45\textwidth}
  ...
  \captionof{figure}{dE/dx Of the simulated cosmic hemisphere both with and without a $\chi^2$/DOF selection criteria. The difference is minimal suggesting that a good series of selection criteria have been found regardless of $\chi^2$/DOF.}
