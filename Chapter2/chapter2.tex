...
In the Standard Model it is easier for the neutrino to be assumed as massless, though there is no fundamental reason for neutrinos to be massless, it simplifies much in the standard model if this is the case \cite{griffiths2008neutrinoOscillations}. However, this assumption though simple and neat is now known to be wrong. In particular, the phenomena of neutrino oscillation where neutrinos oscillate between different mass and flavour states proves that neutrinos cannot be massless \cite{griffiths2008neutrinoOscillations}. The first experimental evidence of neutrino oscillations was the solar neutrino problem where the predicted rate by the Standard Solar Model (SSM) by Bahcall in 1968 did not match the measured rate from the previously mentioned Homestake Mine experiment \cite{griffiths2008neutrinoOscillations}. Bahcall attempted to model the expected neutrino rate for the $^{37}$Cl the predicted rate was $3.5 \times 10 ^{-35} \ \textrm{sec}^{-1} \ \textrm{per} \ ^{37}\textrm{Cl} \ \textrm{atom}$ \cite{bahcall1968present}. This conflicted with the measured result from the Homestake Mine of 2.0 $\pm$ 1.2 $\times$ 10 $^{-35}$ sec$^{-1}$ per $^{37}$Cl atom \footnote{Boron 8 electron neutrino production is preferred due to the higher energy of the neutrinos produced whilst still maintaining a reasonably high flux \cite{griffiths2008neutrinoOscillations}} \cite{davis1968homestake}. At the time the large uncertainty on each of these predictions provided some possibility of reconciliation.  
\\\\However, as the Homestake experiment continued to take measurements a discrepancy of about one third became more pronounced over time \cite{griffiths2008neutrinoOscillations}. The Homestake experiment continued to take data until 1995 and produced the result $\Phi$Cl = 2.56 $\pm$ 0.16 Solar Neutrino Unit (SNU) \cite{Bellerive:2003rj}. The SSM continued to be improved and the predicted rate for $\Phi$Cl was 7.6$^{+ 1.3}_{-1.1}$ SNU \cite{Bellerive:2003rj}. An SNU (solar-neutrino unit) is equal to 10$^{-36}$ captures per atom per second \cite{bowles1997russian}, this unit facilitates the comparison of results between different radiochemical experiments \cite{bowles1997russian}. But chlorine is not the only way in which neutrinos can be measured. The Sudbury Neutrino Observatory (SNO) used heavy water, heavy water is when deuterium is used in place of regular hydrogen to capture neutrinos, D$_2$O as opposed to H$_2$O. This heavy-water allows SNO to observe the measure neutrino interaction in 3 unique ways which will be expanded upon shortly. In addition Kamiokande and later Super-Kamiokande (SuperK) experiments would use normal water to measure neutrino interactions \cite{Bellerive:2003rj}. These experiments are sensitive to different parts of the solar neutrino spectrum with SuperK and SNO being more sensitive to higher energies than the chlorine experiments \cite{Bellerive:2003rj}. The SuperK experiment used neutrino scattering as described by equation \ref{equ:neutrino_scattering} to detect any type of neutrino but is 6.5 times more sensitive to electron neutrinos \cite{griffiths2008neutrinoOscillations}. SNO also works through neutrino scattering but also works through the charged-current (CC) interaction as described by equation \ref{equ:charged-current_reaction} and the neutral-current (NC) interaction as described by equation \ref{equ:neutral-current_reaction} \cite{sno2001}\cite{Bellerive:2003rj} \cite{griffiths2008neutrinoOscillations}. 
\begin{equation}
    \nu + e \rightarrow  \nu + e \hspace{0.5cm}\textrm{(ES)}
    \label{equ:neutrino_scattering}
\end{equation}
\begin{equation}
    \nu_e + d \rightarrow p + p + e \hspace{0.5cm}\textrm{(CC)}
    \label{equ:charged-current_reaction}
\end{equation}
\begin{equation}
    \nu + d \rightarrow  n + p + \nu \hspace{0.5cm}\textrm{(NC)}
    \label{equ:neutral-current_reaction}
\end{equation}
\\By combining the results from SuperK which uses exclusively neutrino scattering and the SNO results for CC it is possible to solve the solar neutrino problem. SuperK found 45.1\,\%~$\pm$~0.5\,\%~(stat)$^{+1.6\,\%}_{-1.4\,\%}$~(syst) according to the BP2000 standard solar model \cite{superK2001}. Whereas the CC interaction results from SNO found 34.7\,\% $\pm$ 2.9\,\% of the expected electron neutrino flux\cite{sno2001}. When comparing the results of SNO and Super-K to the BPB01 solar model (figure \ref{fig:sno_superK_comparision_plot}) a discrepancy of $\sim$ 10\,\% is observed. But this does not mean 10\,\% of the neutrinos were muon neutrinos and tau neutrinos, as the SuperK detector is 6.5 times more sensitive to electron neutrinos. 10\,\% $\times$ 6.5 = 65\,\%, therefore 65\,\% of the neutrinos are muon and tau flavoured neutrinos. And so all neutrinos are now accounted for as the Cl results from SNO accounted for the other 35\,\% and thus the solar neutrino problem was deemed solved  \cite{griffiths2008neutrinoOscillations}. However, this did require using data from two different detectors. A year later SNO would publish their results using ES, CC, and NC fluxes, in particular the use of the NC flux was especially important as the NC is equally sensitive to all active neutrino flavours \cite{ahmad_snoNC_2002}. When comparing the fluxes in equations \ref{equ:sno_cc_flux}~--~\ref{equ:sno_nc_flux} the value of NC flux was significantly higher than if neutrino oscillations were not occurring \cite{ahmad_snoNC_2002}. This is important as it shows the electron neutrinos are not decaying into a lighter undiscovered particle but are changing flavour into the muon and tau neutrinos \cite{griffiths2008neutrinoOscillations}. When the fluxes from equations \ref{equ:sno_cc_flux}~--~\ref{equ:sno_nc_flux} are split between the electron and nonelectron components the nonelectron flux is measured to be 3.41$^{+0.66}_{-0.64}\,\times\,10^6\,\textrm{cm}^{-2}\,\textrm{s}^{-1}$ which is $5.3\,\sigma$ providing strong evidence for flavor transformation consistent with neutrino oscillations \cite{ahmad_snoNC_2002}. These results showed that neutrino oscillations had a clear experimental basis for their existence proving that the solar neutrinos were changing flavours as they traversed through space. 
\begin{equation}
    \phi^{\textrm{SNO}}_{\textrm{CC}} = 1.76^{+0.06}_{-0.05}(\textrm{stat})^{+0.09}_{-0.09}(\textrm{syst}) \,\times\,10^6\,\textrm{cm}^{-2}\,\textrm{s}^{-1}
    \label{equ:sno_cc_flux}
\end{equation}
\begin{equation}
    \phi^{\textrm{SNO}}_{\textrm{ES}} = 2.39^{+0.24}_{-0.23}(\textrm{stat})^{+0.12}_{-0.12}(\textrm{syst})
    \,\times\,10^6\,\textrm{cm}^{-2}\,\textrm{s}^{-1}
    \label{equ:sno_es_flux}
\end{equation}
\begin{equation}
    \phi^{\textrm{SNO}}_{\textrm{NC}} = 5.09^{+0.44}_{-0.43}(\textrm{stat})^{+0.46}_{-0.43}(\textrm{syst}) 
    \,\times\,10^6\,\textrm{cm}^{-2}\,\textrm{s}^{-1}
    \label{equ:sno_nc_flux}
\end{equation}
\begin{figure}[!h]
 \centering
 ...
 \captionof{figure}{The ratio of measured SNO and Super-K data to the expected kinetic energy distribution from the BPB01 model (the updated version of previously mentioned BP2000 model). The correlated systematic errors for the $^8$B charged current interactions are included. Notice the $\sim$ 10 \,\% discrepancy in data between the super K elastic scattering flux (in shaded grey) and the SNO charged current flux (the dashed line). From \cite{sno2001}.} %~can be used as a kind of place holder in latex
 \label{fig:sno_superK_comparision_plot}
\end{figure}
...
The approach that will be taken to explain neutrino oscillations is to treat the two systems as a linear combination of flavour (weak) eigenstates ($\ket{\nu_e},\ket{\nu_\mu}$) which is considered the ``standard quantum mechanical treatment'' \cite{sassaroli1999neutrino}  \cite{griffiths2008neutrinoOscillations}. If oscillations occur then a single flavour cannot account for all the energy in a given system. Neither flavour is an eigenfunction of the Hamiltonian \cite{griffiths2008neutrinoOscillations}.
\\\\As a result, the mass eigenstates that neutrinos propagate through space as (labelled $\ket{\nu_1}$ and $\ket{\nu_2}$ here) are therefore a combination of the flavour eigenstates (which are the eigenstates neutrinos interact as):
\begin{equation}
    \begin{bmatrix}
        \ket{\nu_1} \\
        \ket{\nu_2}
...
\end{equation}
The sines and cosines are a way to enforce normalisation between the two mass eigenstates \cite{griffiths2008neutrinoOscillations}. We then introduce the amplitudes for detecting an electron neutrino ($C_e$) and a for detecting a muon neutrino ($C_\mu$). Then the two amplitudes ($C_1$, $C_2$) for the neutrino energy states ($E_1$, $E_2$) are introduced \cite{sassaroli1999neutrino}. These two amplitudes evolve over time as:
...
U
    ...
    \label{PMNS_matrix}
\end{equation}
Where $s_{ij}$ and $c_{ij}$ represent $\sin(\theta_{ij})$ and $\cos(\theta_{ij})$ respectively. The equation is also parameterized by the CP violation phases $\delta$ and $\alpha$ which represent the Dirac and Majorana phases respectively \cite{Olive_2014}. However, at this time experiments to prove the Majorana over Dirac neutrinos have not proven fruitful. A key experimental observation to prove Majorana neutrinos over Dirac neutrinos would be neutrino-less double beta decay. Neutrino-less double beta decay is where two neutrons decay into two protons emitting two electrons but no neutrinos thus violating lepton number conservation \cite{Cardani_2019}. But neutrino-less double beta decay has not been observed due to the challenging theoretical and experimental requirements (though experiments are improving their precision) \cite{Cardani_2019}.  So the above PNMS matrix is often written without the Majorana component ($\text{diag}(1, e^{\frac{i\alpha21}{2}} , e^{\frac{i\alpha31}{2}})$). 
